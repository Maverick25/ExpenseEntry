...
public class SectionJobAdapter<T> extends BaseAdapter implements Finals
{
	
	 // Debug
   static final boolean DEBUG = false;
   static final String TAG = SectionJobAdapter.class.getSimpleName();
    ...
    */
   public SectionJobAdapter(Context context, BaseAdapter listAdapter, 
   ...
   private String sectionTitleForPosition(int position) {
	...
	public SectionJobAdapter<Job> filterSections(ArrayList<Job> finalObjects,CharSequence charSequence)
	{
	   	if (charSequence.equals(NOTHING))
	   	{
	   		...
	   		return new SectionJobAdapter<Job>(mContext, mListAdapter, R.layout.jobs_section, R.id.list_item_section_text, customerSectionizer);
	   	}
	   	else
	   	{
	   		ArrayList<Object> objects = new ArrayList<Object>();
	   		charSequence = charSequence.toString().toLowerCase();
	   		for (Job job : (ArrayList<Job>) finalObjects)
	   		{
	   			String lowerJobName = job.getName().toLowerCase();
	   			String lowerCustomerName = job.getCustomerName().toLowerCase();
	   			if (lowerJobName.contains(charSequence) || lowerCustomerName.contains(charSequence) || ((String)(job.getId()+NOTHING)).contains(charSequence))
	   			{
	   				objects.add(job);
	   			}
	   		}
	   		ArrayAdapter<Object> jobs = new ArrayAdapter<Object>(mContext, R.layout.simple_selection_row, objects);
	   		Sectionizer<Job> customerSectionizer = new Sectionizer<Job>() 
						{		
							@Override
							public String getSectionTitleForItem(Job instance) 
				...
				return new SectionJobAdapter<Job>(mContext, jobs, R.layout.jobs_section, R.id.list_item_section_text,customerSectionizer);
	   	}
	}
